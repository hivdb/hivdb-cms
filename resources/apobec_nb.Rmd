...
APOBECS_2019_CSV = "~/Dropbox/APOBECs/APBOECs20190812.csv"
APOBECS_2019_JSON = "~/Dropbox/APOBECs/APBOECs20190812.json"
APOBEC_DRMS_JSON = "~/Dropbox/APOBECs/APBOEC_DRMs20190812.json"
...
```{r}
apobecs_2019 = step_3b2
write.table(
  data.frame(
    apobecs_2019$gene,
    apobecs_2019$codon_pos,
    apobecs_2019$mutated_aa
  ),
  file = APOBECS_2019_CSV,
  sep = ",",
  row.names = F,
  col.names = F
)
write_json(data.frame(
  gene = apobecs_2019$gene,
  position = apobecs_2019$codon_pos,
  hm = apobecs_2019$mutated_aa
), APOBECS_2019_JSON)
...
List of the `r nrow(apobecs_2019)` APOBEC mutations:
```{r}
  human_apobecs_2019 = human_apobec_list[human_apobec_list$Lookup %in% apobecs_2019$lookup_key,]
  human_apobecs_2019 = subset(human_apobecs_2019, select = -c(Lookup))
  colnames(human_apobecs_2019) = c('Gene', 'Pos', 'Mut', '# Pt', '% TgtAA', '% TgtCodon', '% MutAA', '# MutAA', '% Stop', '% Major Subtype', '% Other Subtype')
  human_apobecs_2019 %>%
